...
class Api:
    def __init__(self, base_url: str, timeout: float = 300):
        ...
        self.session = build_opener(HTTPCookieProcessor())
        self.signed_in = False

    def login(self):
        credentials = {'username': 'admin', 'password': os.environ.get('WSC_WORKSPACE_PASSWORD', '123456')}
        req = Request(self.base_url + '/api/workspace/login', data=json.dumps(credentials).encode('utf-8'),
                      headers={'Content-Type': 'application/json'}, method='POST')
        with self.session.open(req, timeout=self.timeout) as response:
            self.signed_in = bool(json.load(response).get('authenticated'))
        if not self.signed_in:
            raise QueueBlocked('Workspace login failed; check WSC_WORKSPACE_PASSWORD.')

    def request(self, method: str, path: str, body=None):
        if method not in {'GET', 'HEAD', 'OPTIONS'} and not self.signed_in:
            self.login()
        ...
        try:
            ...
            if exc.code != 401:
                raise
            # A 401 is rejected before the job handler, so renewing an expired
            # session and retrying once cannot submit a second OCR job.
            self.login()
            with self.session.open(req, timeout=self.timeout) as response:
                return json.load(response)
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--state", type=Path, default=Path(__file__).resolve().parents[1] / "data/library/ocr-queue.json")
