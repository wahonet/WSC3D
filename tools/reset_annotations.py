...
用法：tools/run.ps1 tools/reset_annotations.py [--all] [--yes]
    --all   连对齐记录也删除（坐标链仍保留在 assets.extra 中）
    --yes   不再交互确认
先停后端再运行更稳妥；运行前会自动把数据库备份到 data/backups/annotations-<时间>.db。
"""
import shutil
import sqlite3
import sys
...
bak = ROOT / 'data/backups' / f"annotations-{time.strftime('%Y%m%d-%H%M%S')}.db"
bak.parent.mkdir(parents=True,exist_ok=True)
with sqlite3.connect(bak) as backup: con.backup(backup)
