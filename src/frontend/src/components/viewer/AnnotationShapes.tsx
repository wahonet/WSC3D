...
import { isValid2DGeometry, type Pt } from '../../lib/geometry'
...
export function AnnoShape({ a, toEl, selected, interactive, onSelect, linkedTint, showLabel }: {
  ...
  const title = <title>{annotationLabel(a)}</title>
  const tagText = showLabel && !isCand ? annotationLabel(a) : ''
...
export function ProjectedShape({ p, toEl, selected, linkedTint, interactive, onSelect, showLabel }: {
  ...
  if (!isValid2DGeometry(p.atype, p.geometry)) return null
