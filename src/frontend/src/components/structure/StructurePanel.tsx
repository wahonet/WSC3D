...
import {
  ChevronDown, ChevronRight, Eye, EyeOff, GitBranch, Link2, ListTree, Plus, Search, Sparkles, Tags, Wand2,
} from 'lucide-react'
import { LEVELS, LEVEL_LABEL, LEVEL_SHORT } from '../../lib/constants'
import { hasReferences } from '../../lib/references'
import { annotationLabel, ancestors, buildTree, descendantIds, flatten, hasGeometry, isStructural, progress, type TreeNode } from '../../lib/tree'
...
export interface StructureRevealRequest { annotationId: number; sequence: number }

export default function StructurePanel({ revealRequest, compact = false }: { revealRequest?: StructureRevealRequest | null; compact?: boolean } = {}) {
  ...
  const hiddenAnnotations = useApp(s => s.hiddenAnnotations)
  const toggleVisibility = useApp(s => s.toggleAnnotationVisibility)
  ...
  const handledReveal = useRef<StructureRevealRequest | null>(null)
  const listRef = useRef<HTMLDivElement>(null)

  const nodes = useMemo(() => stoneAnnos.filter(a => isStructural(a) && (!compact || hasGeometry(a))), [stoneAnnos, compact])
  ...
  // Explicit navigation may reveal a node hidden by the user's last filter.
  // Keep the mounted editor and its unfinished add/skeleton drafts intact.
  useEffect(() => {
    if (!revealRequest || handledReveal.current === revealRequest) return
    const target = byId.get(revealRequest.annotationId)
    if (!target) return
    ...
    setCollapsed(previous => {
      ...
      for (const ancestor of ancestors(byId, target)) next.delete(ancestor.id)
      return next
    })
    setRevealTarget(target.id)
  }, [revealRequest, byId])

  useEffect(() => {
    if (revealTarget == null || filtering || !rows.some(row => row.a.id === revealTarget)) return
    const frame = requestAnimationFrame(() => {
      const row = listRef.current?.querySelector<HTMLElement>(`[data-annotation-id="${revealTarget}"]`)
      row?.scrollIntoView({ block: 'nearest' })
      row?.focus({ preventScroll: true })
      setRevealTarget(null)
    })
    return () => cancelAnimationFrame(frame)
  }, [revealTarget, filtering, rows])

  // 选中节点时自动展开它的祖先，保证在树里可见
  useEffect(() => {
    if (!selected) return
    if (compact && filter === 'candidate' && selected.review_status !== 'candidate') setFilter('all')
    if (compact) {
      requestAnimationFrame(() => listRef.current?.querySelector<HTMLElement>(`[data-annotation-id="${selected.id}"]`)?.scrollIntoView({ block: 'nearest' }))
    }
  ...
  /* ---------- 拖拽换父 ---------- */
  const onDragStart = (a: Annotation, e: DragEvent) => { setDragId(a.id); e.dataTransfer.setData('application/x-wsc-annotation', String(a.id)); e.dataTransfer.effectAllowed = 'all' }
  ...
  return (
    <div className={`stree${compact ? ' compact' : ''}`}>
      <div className="stree-bar">
        <div className="tree-search" style={{ padding: 0, flex: 1 }}>
          <div className="wrap">
            <Search size={13} />
            <input className="input sm" placeholder={compact ? '搜索标注' : '搜名称 / 层级'} value={query} onChange={e => setQuery(e.target.value)} />
          </div>
        </div>
      ...
      <div className="stree-filters">
        {(compact ? FILTERS.filter(([key]) => key === 'all' || key === 'candidate') : FILTERS).map(([k, lb]) => {
          ...
          return <Chip key={k} size="sm" on={filter === k} onClick={() => setFilter(k)}>{compact && k === 'candidate' ? '待标注' : lb} {n}</Chip>
        })}
        <span style={{ flex: 1 }} />
      ...
      {batchIds.length > 0 && (
        <div className="stree-batch">
          ...
          {!compact && <select className="select sm" style={{ width: 92 }} defaultValue="" onChange={e => { if (e.target.value) batchLevel(e.target.value as Level); e.target.value = '' }}>
            <option value="">设层级…</option>
            {LEVELS.map(l => <option key={l.id} value={l.id}>{l.label}</option>)}
          </select>}
          {!compact && <Button size="xs" onClick={() => runAutoParent(batchIds)} title="按几何包含归类所选">归类</Button>}
          {confirmDel ? (
            <>
      ...
      {dragId != null && <div className={`stree-root${overId === 'root' && canDrop('root') ? ' over' : ''}`}
        onDragOver={e => { if (canDrop('root')) { e.preventDefault(); setOverId('root') } }}
        onDragLeave={() => setOverId(null)} onDrop={e => onDrop('root', e)}>
        移到顶层
      </div>}

      <div className="stree-list" ref={listRef}>
        {nodes.length === 0 && (
          <Empty icon={<ListTree size={22} />} title="暂无分割区域">
          </Empty>
        )}
        ...
        {rows.map(({ a, children, depth }) => {
          ...
          return (
            <div key={a.id} data-annotation-id={a.id} tabIndex={-1}
              className={`tn${on ? ' on' : ''}${isCand ? ' cand' : ''}${hiddenAnnotations.includes(a.id) ? ' layer-hidden' : ''}${a.review_status === 'rejected' ? ' rejected' : ''}${dropOk ? ' over' : ''}${dragId === a.id ? ' dragging' : ''}`}
              ...
              {hasGeometry(a) && <button className="layer-toggle" aria-label={`${hiddenAnnotations.includes(a.id) ? '显示' : '隐藏'}标注 ${a.id}`}
                aria-pressed={!hiddenAnnotations.includes(a.id)} onClick={e => { e.stopPropagation(); toggleVisibility(a.id) }}
                onDoubleClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()}>
                {hiddenAnnotations.includes(a.id) ? <EyeOff size={14} /> : <Eye size={14} />}
              </button>}
              ...
              <span className="lb">
                {compact ? annotationLabel(a) : a.label}
