 ...
 * 没有匹配时可现场新增（选一个末级分类）。
 */
export default function ConceptPicker({ value, onChange, defaultCategory, simple = false }: {
  value: number[]
  ...
  const createNew = async () => {
    const name = q.trim()
    if (!name) return
    const c = await addConcept(name, simple ? '' : newCat)
    if (c) add(c)
  }
      ...
      <div className="cpick-chips">
        {chosen.map(c => (
          <span key={c.id} className="cchip" title={`${catPath(c.category_id)}${c.description ? `\n${c.description}` : ''}`}>
            ...
            <button onClick={() => onChange(value.filter(i => i !== c.id))} aria-label={`移除概念 ${c.name}`}><X size={10} /></button>
          </span>
        ))}
        <input className="cpick-input" aria-label="搜索概念" placeholder={chosen.length ? '再加一个…' : '搜索或输入概念'} value={q}
          onFocus={() => setOpen(true)} onChange={e => { setQ(e.target.value); setOpen(true) }}
          onKeyDown={e => {
            if (e.key === 'Enter') {
              e.preventDefault()
              if (simple && q.trim() && !exact) createNew()
              else if (hits.length > 0) add(hits[0])
        ...
        <div className="cpick-menu">
          {hits.map(c => (
            <button key={c.id} className="cpick-item" onMouseDown={e => e.preventDefault()} onClick={() => add(c)}>
              ...
              {!simple && <span className="cat">{catPath(c.category_id)}</span>}
          ...
          {q.trim() && !exact && (
            <div className="cpick-new">
              <Plus size={12} />
              <span>新增「<b>{q.trim()}</b>」{!simple && '到'}</span>
              {!simple && <select className="select sm" value={newCat} onChange={e => setNewCat(e.target.value)}>
                {leafCats.map(c => <option key={c.id} value={c.id}>{catPath(c.id)}</option>)}
              </select>}
              <button className="btn xs primary" onMouseDown={e => e.preventDefault()} onClick={createNew}>添加</button>
            </div>
          )}
        </div>
      )}
