...
import { annotationLabel, buildTree, flatten } from '../../lib/tree'
import { stageReferenceExcerpt } from '../../lib/annotationDraft'
import { useApp } from '../../store/useApp'
import { toast } from '../../store/useToast'
import type { DocFigure, DocSegment, PageDetail, ReferenceExcerpt, SegmentKind, SegmentReview } from '../../types'
import { Badge, Button, Chip, Empty, Spinner } from '../ui'
import { Mark, splitWords } from './highlight'
import './library-references.css'
import PageAnnotations, { usePageAnnotations } from './PageAnnotations'
...
interface ReferenceAction {
  ...
  targetLabel: string
  onLink: () => void
  onExcerpt?: (excerpt: ReferenceExcerpt) => void
}
...
export default function PageWorkbench({ pageId, onSelectPage, onLoaded, focus = null, highlight = '' }: {
  ...
  const backlinks = usePageAnnotations(pageId)
  ...
  const referenceAction = (kind: 'segment' | 'figure', sourceId: number): ReferenceAction => ({
    ...
    onExcerpt: target ? excerpt => { void (async () => {
      try {
        const updated = await addAnnotationReference(target.id, kind === 'segment' ? { kind, segment_id: sourceId } : { kind, figure_id: sourceId })
        const ref = updated.references.find(item => item.kind === kind && (kind === 'segment' ? item.segment_id : item.figure_id) === sourceId && !item.source_missing)
          ...
          <div className="pwb-imgwrap">
            ...
            {imgSize && (
              <svg className="pwb-overlay" width={imgSize.w} height={imgSize.h}>
                ...
                {segments.map(s => (
                  <Box key={s.id} id={s.id} bbox={s.bbox} size={imgSize} color={backlinks.rows.find(row => row.references.some(ref => ref.segment_id === s.id))?.annotation.color || KIND_COLOR[s.kind] || '#999'}
                    label={backlinks.rows.some(row => row.references.some(ref => ref.segment_id === s.id)) ? '关联图像' : undefined} on={s.id === sel} onClick={() => selectSegment(s.id)} />
                ))}
              </svg>
            )}
          </div>
        ...
        <div className="pwb-reference-column">
          <div className="pwb-reference-target">
            ...
            <select id="library-reference-node" className="select sm" value={target?.id ?? ''} disabled={!nodes.length}
              ...
              {nodes.map(({ a, depth }) => <option key={a.id} value={a.id}>{'　'.repeat(depth)}{annotationLabel(a)} · #{a.id}</option>)}
            </select>
          </div>
          <PageAnnotations rows={backlinks.rows} error={backlinks.error} />
...
function SegmentCard({ s, words, on, onSelect, onSaved, reference }: { s: DocSegment; words: string[]; on: boolean; onSelect: () => void; onSaved: () => void; reference: ReferenceAction }) {
  const [edit, setEdit] = usePersistentDraft(`segment.${s.id}`, s.text_edit)
  ...
  return (
    <div className={`segc${on ? ' on' : ''} rv-${s.review_status}`} data-seg={s.id} onClick={onSelect}>
      ...
      <div className="segc-machine">{reference.onExcerpt && !s.text_edit
        ? <ExcerptText text={s.text} onPick={reference.onExcerpt} disabled={dirty || reference.busy} />
        : s.text ? <Mark text={s.text} words={words} /> : <span className="muted">（空）</span>}</div>
      {on && (
      ...
      {s.text_edit && <div className="segc-edited">校：<ExcerptText text={s.text_edit} onPick={reference.onExcerpt} disabled={dirty || reference.busy} /></div>}
...
function FigureCard({ f, on, onSelect, onSaved, reference }: { f: DocFigure; on: boolean; onSelect: () => void; onSaved: () => void; reference: ReferenceAction }) {
  const [caption, setCaption] = usePersistentDraft(`figure.${f.id}.caption`, f.caption)
  const [label, setLabel] = usePersistentDraft(`figure.${f.id}.label`, f.label)
