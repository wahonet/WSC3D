...
import type { LibraryCollection, SearchHit, SearchOut } from '../../types'
 ...
 */
export default function LibrarySearch({ collection, activeSegment, onOpenHit, onActiveChange }: {
  ...
  const run = useCallback(async (query: string, doc: number | null, offset = 0) => {
    ...
    setBusy(true)
    try {
      const r = await searchLibrary(query, { collection, documentId: doc, limit: PAGE, offset })
