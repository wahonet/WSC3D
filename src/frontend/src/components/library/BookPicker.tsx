...
import type { DocumentInfo, LibraryCollection } from '../../types'
 ...
 */
export default function BookPicker({ collection, docs, docId, runningDocId, busy, onSelect, onScan }: {
  ...
  return (
    <div className="bpick" ref={rootRef}>
      <div className="bpick-row">
            ...
            {cur ? (
              <>
                ...
                <span className="m">
                  ...
                  <span className="mono">{cur.pages_done}/{cur.page_count} 已有文字</span>
        ...
        <Button size="sm" variant="ghost" icon={<RefreshCw size={13} />} onClick={onScan} disabled={busy} title={collection === 'extension' ? '登记扩展文献并核对原件' : '检查核心10本原件状态'} />
      </div>
      {open && (
        <div className="bpick-menu">
          ...
          <div className="bpick-foot">
            <span className="hint">共 {docs.length} 本{collection === 'core' ? '核心文献' : '扩展文献'}</span>
          </div>
        </div>
      )}
    </div>
