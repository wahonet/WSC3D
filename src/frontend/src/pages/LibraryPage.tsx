...
import type { LibraryCollection, PageDetail, SearchHit } from '../types'
 ...
 */
export default function LibraryPage({ collection = 'core', booksOnly = false }: { collection?: LibraryCollection; booksOnly?: boolean }) {
  const storageKey = collection === 'core' ? 'stonelab.library' : 'stonelab.library.extension'
  const initialHash = useRef(new URLSearchParams(location.hash.replace(/^#/, '')))
  const hashMatches = (initialHash.current.get('scope') === 'extension' ? 'extension' : 'core') === collection
  ...
  const entranceRef = useEntranceMotion<HTMLDivElement>(mode, { duration: 280, skipInitial: true })
  const [docId, setDocId] = useState<number | null>(() => (hashMatches && Number(initialHash.current.get('doc'))) || Number(localStorage.getItem(storageKey + '.doc')) || null)
  ...
  const books = useDefaultLayout({ id: 'stonelab.layout.library-books.' + collection, storage: localStorage })

  const switchMode = useCallback((m: Mode) => {
    if (booksOnly) m = 'books'
    localStorage.setItem(storageKey + '.mode', m)
    const q = new URLSearchParams(location.hash.replace(/^#/, ''))
    q.set('lib', m)
    history.replaceState(null, '', `#${q.toString()}`)
    setMode(m)
  }, [booksOnly, storageKey])
  const selectDoc = useCallback((id: number | null) => {
    const ticket = ++navigation.current
    localStorage.setItem(storageKey + '.doc', String(id ?? ''))
    setDocId(id)
    setPageId(null)
    setFocus(null)
    clearReferenceHash()
    if (id) {
      const pageNo = Number(localStorage.getItem(storageKey + '.page.' + id)) || 1
      listDocPages(id, pageNo - 1, 1).then(rows => {
        if (ticket === navigation.current && rows[0]) setPageId(rows[0].id)
      }).catch(toast.error)
    }
  }, [storageKey])
  ...
  const openReference = useCallback((r: ReferenceLocation) => {
    if (r.collection && r.collection !== collection) return
    switchMode('books')
    const documentId = r.documentId
    if (documentId == null || documentId <= 0) return
    const ticket = ++navigation.current
    localStorage.setItem(storageKey + '.doc', String(documentId))
    setDocId(documentId)
    setHighlight('')
  ...
  }, [switchMode, collection, storageKey])
  ...
  useEffect(() => {
    ...
    const onHash = () => {
      ...
      const scope = q.get('scope') === 'extension' ? 'extension' : 'core'
      if (scope === collection && Number(q.get('doc')) > 0) openReference({ collection, documentId: Number(q.get('doc')), pageNo: Number(q.get('pg')) || 1, segmentId: Number(q.get('seg')) || null, figureId: Number(q.get('fig')) || null })
    }
  ...
  }, [openReference, switchMode, collection])
  useEffect(() => {
    const q = initialHash.current
    const matches = (q.get('scope') === 'extension' ? 'extension' : 'core') === collection
    const d = (matches && Number(q.get('doc'))) || Number(localStorage.getItem(storageKey + '.doc'))
    const pg = (matches && Number(q.get('pg'))) || Number(localStorage.getItem(storageKey + '.page.' + d)) || 1
    if (d > 0 && (booksOnly || q.get('lib') === 'books')) openReference({
      documentId: d, pageNo: pg,
      segmentId: Number(q.get('seg')) || null, figureId: Number(q.get('fig')) || null,
    })
    return () => { navigation.current++ }
  }, [openReference, booksOnly, collection, storageKey])
  // 校勘台每载入一页就把 doc / pg 回写到深链，刷新或分享都落在正在看的这一页
  const onPageLoaded = useCallback((p: PageDetail) => {
    localStorage.setItem(storageKey + '.page.' + p.document_id, String(p.page_no))
  ...
  const openHit = useCallback((h: SearchHit) => {
    navigation.current++
    localStorage.setItem(storageKey + '.doc', String(h.document_id))
  ...
  if (mode === 'books') {
    return (
      <div className="body" ref={entranceRef}>
        <Group orientation="horizontal" id="library-books" defaultLayout={books.defaultLayout} onLayoutChanged={books.onLayoutChanged}>
          <Panel id="shelf" className="panel-clip" defaultSize="28%" minSize="300px" maxSize="45%">
            <aside className="side">
              <Pane title={booksOnly ? (collection === 'core' ? '文献库' : '扩展库') : tabs}>
                <ErrorBoundary area="书库">
                  <BookShelf collection={collection} docId={docId} pageId={pageId} activeSegment={focus?.segmentId ?? null} onSelectDoc={selectDoc} onSelectPage={selectPage}
