import type {
  Annotation, AnnotationReference, Concept, DocFigure, DocSegment, DocumentInfo, ExemplarBox, GeometryIntent, Level, LibraryCollection, OcrEngine, OcrJob,
...
export const getStone = (id: string) => api.get<StoneInfo>(`/stones/${id}`)
export const stoneAnnotations = (stoneId: string) => api.get<Annotation[]>(`/stones/${stoneId}/annotations`)
...
export const patchStone = (stoneId: string, body: StonePatchBody) =>
  api.patch<StoneInfo>(`/stones/${stoneId}`, body)
export const patchLayer = (stoneId: string, seq: number, summary: string, name?: string) =>
  api.patch<StoneInfo>(`/stones/${stoneId}/layers/${seq}`, { summary, name })
export const setMaster = (stoneId: string, assetId: number) =>
...
export interface AnnotationPatchBody {
  ...
  semantics?: Semantics
  concept_ids?: number[]
  reference_excerpts?: import('./types').AnnotationExcerptPatch[]
  /** 三者齐全即给节点挂接（替换）几何 */
...
/* ---- 结构：自动归类 / 骨架 ---- */
export const autoParent = (stoneId: string, body: { ids?: number[]; only_orphans?: boolean; min_ratio?: number; include_candidates?: boolean }) =>
  api.post<{ assigned: number; skipped: number; details: string[] }>(`/stones/${stoneId}/structure/auto-parent`, body)
export const skeletonPreview = (stoneId: string) =>
  api.get<{ items: SkeletonItem[]; asset_id: number | null }>(`/stones/${stoneId}/structure/skeleton`)
export const skeletonCreate = (stoneId: string, items: SkeletonItem[], assetId: number | null) =>
...
/* ---- 文献库 / OCR ---- */
export const libraryScan = (collection: LibraryCollection = 'core') =>
  api.post<{ documents: number; added: number; updated: number; removed: number; imported_pages: number; skipped: { book_id: string; reason: string }[] }>(`/library/scan?collection=${collection}`)
export const listDocuments = (collection: LibraryCollection = 'core') => api.get<DocumentInfo[]>(`/library/documents?collection=${collection}`)
...
export const getDocumentPage = (docId: number, pageNo: number) => api.get<PageDetail>(`/library/documents/${docId}/pages/${pageNo}`)
...
export const searchLibrary = (q: string, opts: { documentId?: number | null; collection?: LibraryCollection; limit?: number; offset?: number } = {}) => {
...
export const alignCommit = (body: {
  stone_id: string; left_asset_id: number; right_asset_id: number; geometry: Record<string, unknown>
