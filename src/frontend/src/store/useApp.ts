...
function readHash(): { assetId: number | null; nodeId: number | null; page: Page; engine: SegEngine | null } {
  ...
  return {
    assetId: a > 0 ? a : null,
    nodeId: Number(q.get('node')) || null,
    page: PAGE_IDS.includes(p as Page) ? (p as Page) : LEGACY_PAGE[p] ?? 'home',
    engine: e && ENGINES.includes(e) ? e : null,   // 仅启动时读取，不回写
  }
}
function writeHash(assetId: number | null, page: Page, clearLibraryAnchors = false) {
  const cur = new URLSearchParams(location.hash.replace(/^#/, ''))
  const q = new URLSearchParams(location.hash.replace(/^#/, ''))
  if (assetId) q.set('a', String(assetId)); else q.delete('a')
  if (page !== 'home') q.set('p', page); else q.delete('p')
  const nodeId = useApp.getState().selectedId
  if (nodeId != null) q.set('node', String(nodeId)); else q.delete('node')
  ...
  if (clearLibraryAnchors && page !== 'library') for (const key of ['seg', 'fig', 'src', 'off']) q.delete(key)
  const h = q.toString()
...
export const useApp = create<AppState>((set, get) => ({
  ...
  hiddenLevels: [],
  hiddenAnnotations: (() => { try { const value = JSON.parse(localStorage.getItem('stonelab.hidden-annotations') || '[]'); return Array.isArray(value) ? value.filter(Number.isInteger) : [] } catch { return [] } })(),
  ...
  boot: async () => {
    ...
    try {
      ...
      const { assetId, nodeId, page, engine } = readHash()
      ...
      if (nodeId && get().curAsset) {
        await get().refreshAnnos()
        if (get().stoneAnnos.some(a => a.id === nodeId)) get().select(nodeId)
      }
    } catch (e) {
      set({ backendOk: false })
      toast.error(e)
  ...
  openAsset: async (stone, asset) => {
    ...
    if (!sameStone || !get().stoneInfo) {
      try { const info = await getStone(stone.id); if (get().curStone?.id === stone.id) set({ stoneInfo: info }) } catch (e) { toast.error(e) }
    }
  },
  ...
  select: id => {
    ...
    writeHash(get().curAsset?.id ?? null, get().page)
  ...
  setPage: p => {
    ...
    writeHash(get().curAsset?.id ?? null, p, true)
  },
  toggleTheme: () => {
  ...
  toggleAnnotationVisibility: id => set(s => {
    const hiddenAnnotations = s.hiddenAnnotations.includes(id) ? s.hiddenAnnotations.filter(value => value !== id) : [...s.hiddenAnnotations, id]
    try { localStorage.setItem('stonelab.hidden-annotations', JSON.stringify(hiddenAnnotations)) } catch { /* in-memory visibility remains available */ }
