...
export function solveSimilarity(L: Pt[], R: Pt[]) {
  const n = L.length
  if (n < 2 || R.length !== n || [...L, ...R].some(p => p.length !== 2 || !p.every(Number.isFinite))) return null
  const ma = [0, 0], mb = [0, 0]
  for (let i = 0; i < n; i++) { ma[0] += L[i][0]; ma[1] += L[i][1]; mb[0] += R[i][0]; mb[1] += R[i][1] }
  ma[0] /= n; ma[1] /= n; mb[0] /= n; mb[1] /= n
  let P = 0, Q = 0, na = 0, nb = 0
  for (let i = 0; i < n; i++) {
  ...
  if (na <= 1e-8 || nb <= 1e-8 || Math.hypot(P, Q) <= Number.EPSILON * Math.sqrt(na * nb)) return null
  const theta = Math.atan2(Q, P)
  const s = Math.hypot(P, Q) / nb
  ...
  const result = { s, theta_deg: (theta * 180) / Math.PI, tx, ty, rmse: Math.sqrt(se / n) }
  return s > 0 && Object.values(result).every(Number.isFinite) && Number.isFinite(1 / s) ? result : null
}

/** Historical malformed records must not break the viewer or focus controls. */
export function isValid2DGeometry(atype: string, geometry: unknown): boolean {
  if (!geometry || typeof geometry !== 'object' || Array.isArray(geometry)) return false
  const g = geometry as Record<string, unknown>
  const number = (value: unknown): value is number => typeof value === 'number' && Number.isFinite(value)
  const point = (value: unknown) => Array.isArray(value) && value.length === 2 && value.every(number)
  if (atype === 'rect') return [g.x, g.y, g.w, g.h].every(number) && (g.w as number) > 0 && (g.h as number) > 0
  if (atype === 'ellipse') return [g.cx, g.cy, g.rx, g.ry].every(number) && (g.rx as number) > 0 && (g.ry as number) > 0
  if (atype === 'polygon') return Array.isArray(g.points) && g.points.length >= 3 && g.points.every(point)
  if (atype === 'point') return point(g.p)
  if (atype === 'line') return point(g.p1) && point(g.p2)
  return false
}
...
export function annotationBounds(a: Pick<Annotation, 'atype' | 'geometry'>): [number, number, number, number] | null {
  if (!isValid2DGeometry(a.atype, a.geometry)) return null
