...
from ..services import library, references
...
def _doc_out(db: Session, d: Document, counts: tuple[int, int, int, int] | None = None) -> DocumentOut:
    if counts is None:
        ...
        done, err, n_seg, n_fig = counts
    return DocumentOut(
        id=d.id, code=d.code, collection=d.collection or "core", book_id=d.book_id or "",
        title=d.title, authors=d.authors, year=d.year, publisher=d.publisher, kind=d.kind,
...
def _collection(value: str) -> str:
    if value not in library.COLLECTIONS:
        raise HTTPException(422, "collection 只能是 core 或 extension")
    return value


@router.post("/scan", response_model=LibraryScanReport, summary="核对核心书原件；collection=extension 时把已登记扩展书目登记为文献（含既有全文导入）")
def scan(collection: str = "core", db: Session = Depends(get_db)):
    return library.scan(db, _collection(collection))


@router.get("/documents", response_model=list[DocumentOut], summary="文献列表（含 OCR 进度；按 collection 区分核心10本 / 扩展库）")
def list_documents(collection: str = "core", db: Session = Depends(get_db)):
    query = db.query(Document)
    if collection != "all":
        query = query.filter(Document.collection == _collection(collection))
    docs = query.order_by(Document.collection, Document.code).all()
    ids = [d.id for d in docs]
    page_counts = {(doc, status): count for doc, status, count in db.query(Page.document_id, Page.status, func.count(Page.id)).filter(Page.document_id.in_(ids)).group_by(Page.document_id, Page.status)}
    segments = dict(db.query(Segment.document_id, func.count(Segment.id)).filter(Segment.document_id.in_(ids)).group_by(Segment.document_id).all())
    figures = dict(db.query(Figure.document_id, func.count(Figure.id)).filter(Figure.document_id.in_(ids)).group_by(Figure.document_id).all())
    return [_doc_out(db, d, (page_counts.get((d.id, 'done'), 0), page_counts.get((d.id, 'error'), 0), segments.get(d.id, 0), figures.get(d.id, 0))) for d in docs]
...
@router.get("/pages/{page_id}/annotations", summary="此页关联的图像标注")
def page_annotations(p: Page = Depends(get_page), db: Session = Depends(get_db)):
    return references.page_annotations(db, p)
...
def search(q: str = Query(..., min_length=1), document_id: int | None = None, collection: str | None = None,
           limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0), db: Session = Depends(get_db)):
    return SearchOut(**library.search(db, q, document_id, limit, offset, _collection(collection) if collection else None))
