...
def stop_runtime(request: Request):
    ...
    if callback is None:
        raise HTTPException(409, '当前服务未通过统一启动器运行')
    callback()
    return {'ok': True}
...
@router.post("/scan", response_model=ScanReport, summary="扫描 resources/stones 入库")
def scan(warm: bool | None = None, db: Session = Depends(get_db)):
    """幂等：新文件入库，按指纹追踪改名，缺失或替换保留原有研究数据。
