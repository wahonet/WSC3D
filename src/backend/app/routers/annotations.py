...
def _new(db: Session, body: AnnotationCreate) -> Annotation:
    try:
        validate_annotation_geometry(body.atype, body.geometry)
    except ValueError as error:
        raise HTTPException(422, str(error)) from error
    if body.tool in ('align', 'measure') and 'review_status' not in body.model_fields_set:
    ...
    from ..services.resource_versions import pin_asset
    pin_asset(db, a)
    if body.atype == "none":
        body.geometry = {}
...
def delete_batch(body: IdList, db: Session = Depends(get_db)):
    ...
    deleting = {x.id: x for x in rows}
    for x in rows:
        structure.rehang_children(db, x, deleting)
    for x in rows:
        db.delete(x)
    db.commit()
...
def patch(body: AnnotationPatch, x: Annotation = Depends(get_annotation),
    ...
    if (body.atype is None) != (body.geometry is None):
        raise HTTPException(422, "修改几何时须同时提供 atype 和 geometry")
    if body.atype is not None:
        try:
            validate_annotation_geometry(body.atype, body.geometry)
        except ValueError as error:
            raise HTTPException(422, str(error)) from error
    excerpt_updates = references.prepare_excerpts(x, body.reference_excerpts, body.semantics)
    if body.clear_link:
    ...
    for ref in x.references:
        if ref.id in excerpt_updates:
            ref.excerpts = excerpt_updates[ref.id]
