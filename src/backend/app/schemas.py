...
class ReferenceExcerpt(BaseModel):
    field: Literal["pre_iconographic", "iconographic"]
    start: int = Field(ge=0)
    end: int = Field(gt=0)
    text: str = Field(min_length=1, max_length=20000)


class AnnotationExcerptPatch(ReferenceExcerpt):
    reference_id: int
...
class AnnotationReferenceOut(BaseModel):
    ...
    excerpts: list[ReferenceExcerpt] = Field(default_factory=list)
...
class AnnotationPatch(BaseModel):
    ...
    reference_excerpts: list[AnnotationExcerptPatch] = Field(default_factory=list, max_length=100)
...
class DocumentOut(BaseModel):
    id: int
    code: str
    collection: str = "core"                 # core 核心10本 / extension 扩展库
    book_id: str = ""                        # 扩展库：对应 extension_books.id
    title: str
    authors: str
    year: str
    publisher: str
...
class LibraryScanReport(BaseModel):
    ...
    removed: int
    duration_ms: int
    imported_pages: int = 0                  # 扩展库登记：导入的既有页文本数
...
class ProjectedOut(BaseModel):
    ...
    invalid_annotation_ids: list[int] = Field(default_factory=list)
...
class ScanReport(BaseModel):
    ...
    duration_ms: int = 0
    assets_missing: list[int] = Field(default_factory=list)
