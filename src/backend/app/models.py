...
class Stone(Base):
    ...
    archive: Mapped[dict] = mapped_column(JSON, default=dict)
...
class Layer(Base):
    ...
    stone_id: Mapped[str] = mapped_column(ForeignKey("stones.id"), index=True)
...
class Asset(Base):
    ...
    source_root: Mapped[str] = mapped_column(String(16), default="research")
...
class AnnotationReference(Base):
    ...
    excerpts: Mapped[list] = mapped_column(JSON, default=list, server_default="[]")
    created_at: Mapped[datetime] = mapped_column(DateTime, default=now)
...
class Document(Base):
    ...
    collection='core' 的原件在 resources/documents/core/originals（核心10本）；
    collection='extension' 的原件由 extension_books 登记，relpath 以 extension/ 开头，相对 resources/documents/extension。
    """
    __tablename__ = "documents"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    reference_identity: Mapped[str] = mapped_column(String(32), default=reference_identity)
    code: Mapped[str] = mapped_column(String(32), unique=True, index=True)     # DOC-001 / EXT-001
    collection: Mapped[str] = mapped_column(String(16), default="core")        # core / extension
    book_id: Mapped[str] = mapped_column(String(64), default="")               # extension_books.id（扩展库）
