...
# 统一平台（2026-09-11 之后）新增的列：无论是否为合并库都需要补齐。
UNIFIED_RULES: list[tuple[str, str, str]] = [
    ("annotation_references", "excerpts", "JSON DEFAULT '[]'"),
    ("annotation_knowledge_links", "reference_ids", "JSON DEFAULT '[]'"),
    ("documents", "collection", "VARCHAR(16) DEFAULT 'core'"),   # 文献中心：core 核心10本 / extension 扩展库
    ("documents", "book_id", "VARCHAR(64) DEFAULT ''"),          # 扩展库文献对应的 extension_books.id
]


def _add_columns(conn, rules: list[tuple[str, str, str]]) -> list[str]:
    applied: list[str] = []
    ...
    for table, col, ddl in rules:
        ...
        if not cols_cache[table] or col in cols_cache[table]:
            continue
...
def migrate(engine: Engine) -> list[str]:
    with engine.begin() as connection:
        unified = connection.execute(text("SELECT 1 FROM sqlite_master WHERE name='unified_migrations'")).first()
        added = _add_columns(connection, UNIFIED_RULES)
        if added:
            connection.execute(text("UPDATE documents SET collection='core' WHERE collection IS NULL OR collection=''"))
            connection.execute(text("UPDATE documents SET book_id='' WHERE book_id IS NULL"))
            log.info("migrated: %s", ", ".join(added))
        if unified:
            for action in ("INSERT", "UPDATE"):
                connection.execute(text(f"CREATE TRIGGER IF NOT EXISTS catalogue_identity_{action.lower()} BEFORE {action} ON stones "
                                        "WHEN NEW.id != NEW.code OR length(NEW.id) != 4 OR NEW.id NOT GLOB '武[0-9][0-9][0-9]' "
                                        "BEGIN SELECT RAISE(ABORT,'文物必须使用唯一馆藏编号'); END"))
            return added
    ...
    applied: list[str] = list(added)
    with engine.connect() as conn:
        applied.extend(_add_columns(conn, RULES))
