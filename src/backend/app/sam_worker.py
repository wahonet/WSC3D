...
- sam3.1    : SAM-3.1 multiplex fp16 safetensors 的 detector 部分灌入图像模型。
              保留原工程的图像 detector 适配（缺 37 键：32 个 RoPE 缓冲为运行时生成，
    ...
    try:
        ...
        if engine == "sam3":
            ...
            note = (f"sam3.1 multiplex fp16 detector ({dev})；"
                    f"保留原工程图像 detector 适配；未覆盖键 {len(missing)}（含运行时 RoPE 缓冲）")
            log('sam3.1 non-RoPE missing keys: ' + ', '.join(k for k in missing if 'freqs_cis' not in k))
