...
数据分层（沿用古籍 OCR 工程的做法）：
- 源 PDF 留在 resources/documents/core/originals（不入库、不复制）；
...
from sqlalchemy import bindparam, insert, text as sql, update
from sqlalchemy.orm import Session

from ..config import settings
from ..resource_paths import resolve_resource
from ..db import SessionLocal
from ..models import AnnotationReference, Document, Figure, Page, Segment, reference_identity
...
COLLECTIONS = ("core", "extension")
EXTENSION_PREFIX = "extension/"          # 扩展库文献 relpath 前缀，其后相对 resources/documents/extension
# 页级 OCR 来源标签：新 OCR 引擎与从原 inventory 导入的既有文本
ENGINE_LABEL = {"text-layer": "文字层", "legacy-ocr": "OCR，需核对原图", "mineru": "OCR，需核对原图", "ndl": "OCR，需核对原图"}


# ================================================================ 文件与页图
def doc_path(d: Document) -> Path:
    if d.relpath.startswith(EXTENSION_PREFIX):
        root = (settings.extension_root).resolve()
        path = (root / d.relpath[len(EXTENSION_PREFIX):]).resolve()
        if root not in path.parents:
            raise HTTPException(409, "扩展文献路径不合法")
        return resolve_resource(path)
    import sqlite3
    from contextlib import closing
    with closing(sqlite3.connect(settings.db_path)) as db:
        row = db.execute("SELECT payload FROM source_manifests WHERE id='core10-pinned-v1'").fetchone()
    if row:
        for item in json.loads(row[0]):
            if item['id'] == d.id:
                root = settings.core_versions_dir
                path = (root / item['relpath']).resolve()
                if root.resolve() not in path.parents:
                    raise HTTPException(409, '文献快照路径不合法')
                if path.is_file():
                    return path
                # Recovery may restore an exact original directly to the library root.
                restored = settings.library_root / d.relpath
                if restored.is_file() and sha256_file(restored) == item['file_sha256']:
                    return restored
                raise HTTPException(409, '文献原页快照缺失，请恢复对应版本原件')
    return resolve_resource(settings.library_root / d.relpath)
...
def render_page_image(d: Document, page_no: int, dpi: int, fmt: str = "jpg") -> Path:
    ...
    out = settings.cache_dir / 'library' / f'doc{d.id}' / 'pages' / f"p{page_no:04d}_{dpi}.{fmt}"
    if out.exists():
        return out
    import pypdfium2 as pdfium
...
def _next_code(db: Session) -> str:
    n = db.query(Document).filter(Document.collection == "core").count()
    while True:
        n += 1
        code = f"DOC-{n:03d}"
...
def scan(db: Session, collection: str = "core") -> dict:
    if collection == "extension":
        return register_extension_books(db)
    t0 = time.perf_counter()
    report = {"documents": 0, "added": 0, "updated": 0, "removed": 0, "duration_ms": 0}
    from sqlalchemy import text
    manifest = db.execute(text("SELECT payload FROM source_manifests WHERE id='core10-files-v1'")).fetchone()
    if manifest:
        registered = json.loads(manifest[0])
        for row in registered:
            path = resolve_resource(settings.library_root / row['relpath'])
            report['documents'] += 1
            if not path.is_file():
                report['removed'] += 1
        ...
        # Admission of additional books and a changed scan version is explicit.
        # Never rebind saved OCR coordinates merely because a PDF was overwritten.
        return report
    ...
    for f in sorted(root.rglob("*.pdf")) + sorted(root.rglob("*.PDF")):
        ...
        if d is None:
            ...
            report["updated"] += 1
        report["documents"] += 1
    for d in db.query(Document).filter(Document.collection == "core").all():
...
# ================================================================ 扩展库登记
def extension_pdf(book: dict) -> tuple[str, Path] | None:
    """已登记扩展书目的 PDF 原件：返回 (relpath, 路径)；没有可读原件时返回 None。"""
    filename = book.get("file")
    if not filename:
        return None
    for rel in (f"books/{book['id']}/{filename}", f"library/{filename}"):
        p = resolve_resource(settings.extension_root / rel)
        if p.is_file():
            return EXTENSION_PREFIX + rel, p
    return None


def _kind_of(value) -> str:
    text = str(value or "")
    if "论文" in text or "期刊" in text or "文章" in text:
        return "article"
    if "图录" in text:
        return "catalog"
    if "专著" in text or "书" in text or "著作" in text:
        return "book"
    return "other"


def _script_of(book: dict) -> str:
    if str(book.get('category') or '') in ('外文与综合', '日文研究') or re.match(r'^[A-Za-zÀ-ÿ]', str(book.get('title') or '').strip()):
        return 'modern'
    if '古籍' in str(book.get('kind') or ''):
        return 'classical'
    try:
        return "classical" if int(str(book.get("year") or "")[:4]) < 1912 else "modern"
    except ValueError:
        return "modern"
...
def register_extension_books(db: Session) -> dict:
    """把 extension_books 中有 PDF 原件的书目登记为扩展库文献（含物理页），与核心书共用校勘台与 OCR。

    首次登记时把原 inventory 已有的文字层 / OCR 页文本导入为每页一个机器文段，
    这样既有全文立即可检索、可查看，之后可按页重新 OCR 覆盖。幂等：已登记的书只核对文件变化。
    """
    if not _register_lock.acquire(blocking=False):
        raise HTTPException(409, "扩展库正在登记中，请稍候再试")
    t0 = time.perf_counter()
    report = {"documents": 0, "added": 0, "updated": 0, "removed": 0, "duration_ms": 0, "imported_pages": 0, "skipped": []}
    try:
        from . import catalogue
        books = catalogue.all_books(db)
        existing = {d.book_id: d for d in db.query(Document).filter(Document.collection == "extension").all()}
        by_relpath = {d.relpath: d for d in db.query(Document).all()}
        numbers = [int(d.code[4:]) for d in existing.values() if d.code.startswith("EXT-") and d.code[4:].isdigit()]
        next_no = max(numbers) if numbers else 0
        for book in sorted(books, key=lambda b: (str(b.get("year") or ""), str(b.get("title") or ""))):
            bid = book["id"]
            found = extension_pdf(book)
            d = existing.get(bid)
            if found is None:
                if d is not None:
                    report["removed"] += 1        # 原件不在了：保留记录与 OCR 结果
                continue
            relpath, path = found
            report["documents"] += 1
            size = path.stat().st_size
            meta = book.get("provenance") or {}
            if d is None:
                other = by_relpath.get(relpath)
                if other is not None:
                    report["skipped"].append({"book_id": bid, "reason": f"同一PDF已登记为 {other.code}"})
                    continue
                try:
                    n = _page_count(path)
                except Exception as e:            # 损坏或加密的 PDF：只记录，不阻断其余书目
                    report["skipped"].append({"book_id": bid, "reason": f"无法读取PDF：{type(e).__name__}"})
                    continue
                next_no += 1
                d = Document(code=f"EXT-{next_no:03d}", collection="extension", book_id=bid,
                             title=str(book.get("title") or bid), authors=str(book.get("author") or ""),
                             year=str(book.get("year") or ""), publisher=str(book.get("publisher") or ""),
                             kind=_kind_of(book.get("kind")), script=_script_of(book), relpath=relpath,
                             sha256=str(meta.get("sha256") or meta.get("file_sha256") or ""), bytes=size, page_count=n,
                             has_text_layer=(settings.extension_root / "books" / bid / "text-pages.json").is_file(),
                             notes=str(book.get("note") or ""))
                db.add(d)
                db.flush()
                if n:
                    db.execute(insert(Page), [{"document_id": d.id, "page_no": i, "reference_identity": reference_identity(),
                                               "width": 0, "height": 0, "image_sha256": "", "status": "pending", "engine": "",
                                               "text": "", "error": "", "stats": {}} for i in range(1, n + 1)])
                report["added"] += 1
                report["imported_pages"] += _import_legacy_text(db, d, book)
                by_relpath[relpath] = d
                existing[bid] = d
            elif d.bytes != size or d.relpath != relpath:
                n = _page_count(path)
                d.relpath, d.bytes, d.page_count = relpath, size, n
                d.sha256 = str(meta.get("sha256") or meta.get("file_sha256") or "") or sha256_file(path)
                have = {p.page_no for p in d.pages}
                missing = [i for i in range(1, n + 1) if i not in have]
                if missing:
                    db.execute(insert(Page), [{"document_id": d.id, "page_no": i, "reference_identity": reference_identity(),
                                               "width": 0, "height": 0, "image_sha256": "", "status": "pending", "engine": "",
                                               "text": "", "error": "", "stats": {}} for i in missing])
                report["updated"] += 1
        db.commit()
    except Exception:
        db.rollback()
    ...
    report["duration_ms"] = int((time.perf_counter() - t0) * 1000)
    log.info("extension library registration: %s", {k: v for k, v in report.items() if k != "skipped"} | {"skipped": len(report["skipped"])})
    return report


def _import_legacy_text(db: Session, d: Document, book: dict) -> int:
    """导入原 inventory 的 text-pages.json / ocr-pages.json：每页一个整页机器文段，文字层优先于旧 OCR。"""
    folder = settings.extension_root / "books" / book["id"]
    page_range = (book.get("provenance") or {}).get("index_page_range")
    values: dict[int, tuple[str, str]] = {}
    for filename, engine in (("ocr-pages.json", "legacy-ocr"), ("text-pages.json", "text-layer")):
        ...
        for row in rows if isinstance(rows, list) else []:
            number = row.get("page") if isinstance(row, dict) else None
            if not isinstance(number, int) or not 1 <= number <= d.page_count:
                continue
            if page_range and not page_range[0] <= number <= page_range[1]:
                continue
            value = str(row.get("text") or "")
            if value.strip():
                values[number] = (value, engine)
    if not values:
        return 0
    stamp = datetime.now()
    pages = {p.page_no: p.id for p in db.query(Page.page_no, Page.id).filter(
        Page.document_id == d.id, Page.page_no.in_(list(values)), Page.status.in_(['pending', 'error']), ~Page.segments.any()).all()}
    segments, page_updates = [], []
    for number, (value, engine) in sorted(values.items()):
        page_id = pages.get(number)
        if page_id is None:
            continue
        segments.append({"document_id": d.id, "page_id": page_id, "seq": 0, "kind": "text", "text": value, "text_edit": "",
                         "bbox": [0, 0, 1, 1], "confidence": None, "review_status": "machine", "revision": 0, "note": "",
                         "reference_identity": reference_identity(), "updated_at": stamp})
        page_updates.append({"id": page_id, "text": value, "status": "done", "engine": engine, "ocr_at": stamp,
                             "stats": {"segments": 1, "figures": 0, "source": "legacy-import"}})
    if not segments:
        return 0
    db.execute(insert(Segment), segments)
    db.execute(update(Page), page_updates)
    # 只写入本次补齐的页，兼容旧维护工具继续补做 OCR，避免重复索引或覆盖校订。
    db.execute(sql("INSERT INTO segments_fts(text, segment_id, document_id, page_no) "
                   "SELECT s.text, s.id, s.document_id, p.page_no FROM segments s JOIN doc_pages p ON p.id = s.page_id "
                   "WHERE s.page_id IN :ids").bindparams(bindparam('ids', expanding=True)), {'ids': [p['id'] for p in page_updates]})
    return len(segments)
...
def _ensure_worker(engine: str) -> Optional[str]:
    ...
    if py is None:
        return (f"找不到 {engine} 的 OCR 工作环境；请用项目启动入口校验并准备 runtime/runtime.zip，"
                f"或设置环境变量 STONELAB_{'OCR' if engine == 'mineru' else 'NDL'}_PYTHON")
    cmd = [py, "-X", "utf8", str(WORKER_SCRIPT), "--engine", engine,
           "--ndl-root", str(settings.ndl_root), "--models-dir", str(settings.ndl_models_dir if engine == "ndl" else settings.mineru_models_dir)]
    try:
...
def _request(engine: str, payload: dict) -> dict:
    if engine == "mineru":
        ...
        with lease("mineru"):
            return _request_raw(engine, payload)
    return _request_raw(engine, payload)


def _request_raw(engine: str, payload: dict) -> dict:
...
def shutdown_workers() -> None:
    for engine in ENGINES:
        shutdown_worker(engine)


def shutdown_worker(engine) -> None:
    for e in [engine]:
        p = _procs.get(e)
        if p is None:
            continue
        try:
            from .process_tree import stop_worker
            stop_worker(p)
        except Exception:
            try:
                p.kill()
...
def patch_segment(db: Session, s: Segment, text_edit: str | None, kind: str | None, review: str | None,
    ...
    expected_revision = s.revision if base_revision is None else base_revision
    if expected_revision != s.revision:
        raise HTTPException(409, f"该文段已被修改（当前版本 {s.revision}），请刷新后再保存")
    changes = {attr: value for attr, value in
               (("text_edit", text_edit), ("kind", kind), ("review_status", review), ("note", note))
               if value is not None and value != getattr(s, attr)}
    try:
        if changes:
            # The predicate is checked by the database, even if this Session
            # read the segment before another editor committed their changes.
            result = db.execute(update(Segment).where(Segment.id == s.id, Segment.revision == expected_revision)
                                .values(**changes, revision=expected_revision + 1)
                                .execution_options(synchronize_session=False))
            if result.rowcount != 1:
                raise HTTPException(409, "该文段已被修改，请刷新后再保存")
        db.refresh(s)
        if not changes and s.revision != expected_revision:
            raise HTTPException(409, f"该文段已被修改（当前版本 {s.revision}），请刷新后再保存")
        if "text_edit" in changes:
            db.execute(sql("DELETE FROM segments_fts WHERE segment_id = :i"), {"i": s.id})
            db.execute(sql("INSERT INTO segments_fts(text, segment_id, document_id, page_no) VALUES (:t, :i, :d, :p)"),
...
def search(db: Session, q: str, document_id: int | None, limit: int = 50, offset: int = 0,
    ...
    - `collection` 限定核心10本或扩展库（文献中心两个标签页各自检索）。
    """
    ...
    coll_filter = " AND {col} IN (SELECT id FROM documents WHERE collection = :c)" if collection in COLLECTIONS else ""
    if use_fts:
        term = " AND ".join('"' + w.replace('"', '""') + '"' for w in words)
        params: dict = {"q": term, "d": document_id, "c": collection, "n": limit, "o": offset}
        base = "FROM segments_fts f WHERE segments_fts MATCH :q" + coll_filter.format(col='f.document_id')
        ...
        rows = db.execute(sql(
            ...
            f"WHERE segments_fts MATCH :q{coll_filter.format(col='f.document_id')}{doc_filter.format(col='f.document_id')} "
        ...
        cond = " AND ".join(f"{body} LIKE :w{i}" for i in range(len(words))) + coll_filter.format(col='s.document_id')
        params = {f"w{i}": f"%{w}%" for i, w in enumerate(words)} | {"d": document_id, "c": collection, "n": limit, "o": offset}
