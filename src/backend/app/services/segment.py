...
真实推理在 app/sam_worker.py 子进程中进行，默认复用主 Python 环境中的
CUDA torch / sam3 / mobile_sam；可用 STONELAB_SAM_PYTHON 指定其他解释器。
...
MOBILESAM_WEIGHT = model_path("mobilesam", "weight")
SAM3_WEIGHT = model_path("sam3", "weight")


def _find_sam31() -> Optional[Path]:
    weight = model_path("sam3.1", "weight")
    return weight if weight.is_file() else None
...
def models_status() -> dict:
    ...
    def w(p: Optional[Path]) -> dict:
        ...
        return {"file": str(p.relative_to(settings.models_root)) if p else None,
                "exists": exists,
...
def text_segment(engine: str, image_path: Path, prompt: str, threshold: float, max_results: int,
    ...
    with lease(engine):
        if _engine_state[engine]['status'] != 'ready':
            loaded = _request({'cmd': 'load', 'engine': engine})
            if not loaded.get('ok'):
                return loaded
            _engine_state[engine] = loaded.get('engines', {}).get(engine, {'status': 'ready', 'detail': ''})
