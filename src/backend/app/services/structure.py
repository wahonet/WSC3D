...
def default_asset(db: Session, stone_id: str) -> Asset | None:
...
# ================================================================ 父级建议 / 自动归类
def suggest_parents(db: Session, stone_id: str, x: Annotation, limit: int = 5) -> list[ParentSuggestion]:
...
def auto_parent(db: Session, stone_id: str, body: AutoParentIn) -> AutoParentOut:
    rows = structural_rows(db, stone_id)
...
def rehang_children(db: Session, x: Annotation, deleting: dict[int, Annotation] | None = None) -> int:
    """挂到最近的存活祖先；批量删除时不修改待删节点的原始父级。"""
    deleting = deleting if deleting is not None else {x.id: x}
    parent_id = x.parent_id
    seen = {x.id}
    while parent_id in deleting:
        if parent_id in seen:
            parent_id = None
            break
        seen.add(parent_id)
        parent_id = deleting[parent_id].parent_id
    kids = db.query(Annotation).filter(Annotation.parent_id == x.id).all()
    kept = 0
    for k in kids:
        if k.id not in deleting:
...
def adopt_geometry(db: Session, target: Annotation, source: Annotation) -> Annotation:
    ...
    try:
        validate_annotation_geometry(source.atype, source.geometry)
    except ValueError as error:
        raise HTTPException(422, str(error)) from error
    if target.id in descendants(structural_rows(db, source.stone_id), source.id):
        raise HTTPException(422, "不能把祖先节点并入自己的子孙节点（会成环）")
