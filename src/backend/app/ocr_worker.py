...
if __package__:
    ...
    from .portable_models import write_mineru_config
...
class MinerUEngine:
    def __init__(self, models_dir: str):
        if not models_dir:
            raise ValueError('请指定项目内的 MinerU 模型目录。')
        self._config_dir = tempfile.TemporaryDirectory(prefix='wsc-mineru-')
        local_config = write_mineru_config(Path(models_dir), Path(self._config_dir.name) / 'mineru.json')
        os.environ['MINERU_MODELS_DIR'] = str(Path(models_dir).resolve())
        os.environ['MINERU_TOOLS_CONFIG_JSON'] = str(local_config)
        os.environ['MINERU_MODEL_SOURCE'] = 'local'
        log(f'MinerU local model configuration: {local_config}')
...
class NdlEngine:
    def __init__(self, root: str, models_dir: str):
        self.root = Path(root)
        weights = Path(models_dir)
        src = self.root / "src"
        if not src.is_dir():
        ...
        self.detector = RTMDet(model_path=str(weights / "rtmdet-s-1280x1280.onnx"),
        ...
        self.recognizer = PARSEQ(model_path=str(weights / "parseq-ndl-32x384-tiny-10.onnx"),
...
# ================================================================ 主循环
def exit_worker(code, engine_obj):
    # os._exit bypasses multiprocessing finalizers. Explicitly stop children so
    # completed MinerU jobs do not keep the old Python executable/model files open.
    ...
    for child in multiprocessing.active_children():
        ...
        if child.is_alive():
            child.kill()
            child.join(timeout=3)
    if isinstance(engine_obj, MinerUEngine):
        engine_obj._config_dir.cleanup()
    os._exit(code)
    ...
    for raw in sys.stdin:
        ...
        try:
            ...
            try:
                ...
                exit_worker(2, engine_obj)
    # 不等待库的后台线程（ray 等），直接退出
    try:
        _PROTO.flush()
    finally:
        exit_worker(0, engine_obj)
