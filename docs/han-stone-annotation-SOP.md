...
>   - v0.3.1：批量标注前的管线加固（A1-A5 + B + D 修复）
>   - **v0.3.2（本版）**：weak / silver / gold 分层、弱监督导出、主动学习队列
...
> **v0.3.2 训练集分层**（vs v0.3.1，2026-05-05）
> - **新增 `annotationQuality`**：`weak` / `silver` / `gold` 三档。弱标注用于 bbox、点、涂鸦和弱监督；silver 用于 AI 或快速修正 mask；gold 用于专家精修验证/论文评估。
> - **新增 `geometryIntent`**：`visible_trace` / `semantic_extent` / `reconstructed_extent`。汉画像石边界常包含专家解释，必须把可见刻痕、语义范围和复原范围分开。
> - **新增 `trainingRole`**：`train` / `validation` / `holdout`。gold 子集可显式留作验证，争议样本可保留但默认不进训练。
> - **新增弱监督与闭环导出**：A2 导出现在额外写 `weak_annotations.json`、`gold_validation.json`、`enhancement_manifest.json`、`baseline_recipes.json`、`active_learning_queue.json` 与 `DATASET_CHANGELOG.md`。
>
...
> - **catalog 健康检查收敛**：当前 catalog 以 `temp/` 模型目录为主数据源，不再依赖旧版
>   metadata 模糊配对流程；`GET /api/catalog/health` 主要用于暴露 numericKey 冲突与目录统计，
>   `data/catalog.override.json` 仅保留为空的历史占位文件，不再作为批量标注前置流程。
>
> **v0.3 主要变化**（vs v0.2）
...
每一条 IIML annotation 在保存前必须能填齐下面槽位（缺 motif 不致命，缺其它硬约束不进正式训练池）：
...
| 标注质量层 | `annotationQuality` | `weak` / `silver` / `gold`，决定训练和评估使用方式 |
| 几何语义 | `geometryIntent` | `visible_trace` / `semantic_extent` / `reconstructed_extent` |
| 训练用途 | `trainingRole` | `train` / `validation` / `holdout` |
| 问题标签 | `annotationIssues[]` | 低对比、纹理混淆、边界歧义、遮挡风化等主动学习信号 |
...
> - `annotationQuality`：训练质量分层，避免把弱标注当成精确 mask 真值
> - `geometryIntent`：边界语义分层，避免把专家复原轮廓和可见刻痕混训
...
### 3.2 标注质量层与几何语义（v0.3.2 新增）

汉画像石的边界经常不是"图像里天然存在的一条线"，而是可见刻痕、石材纹理、专家判读和复原想象的叠加。因此训练集不再要求所有 annotation 都是同一质量的精确 mask。

| 字段 | 值 | 用法 |
| --- | --- | --- |
| `annotationQuality` | `weak` | BBox、Point、LineString/涂鸦、局部线索；用于快速覆盖、检测、BoxInst/点/涂鸦弱监督 |
| `annotationQuality` | `silver` | SAM/HQ-SAM/人工快速修正后的 polygon/mask；可训练，但评估报告必须单独统计 |
| `annotationQuality` | `gold` | 专家精修、边界和类别均可信；优先留作 validation/test 或论文评估 |
| `geometryIntent` | `visible_trace` | 只圈可见刻痕/浮雕痕迹，适合边缘和微痕研究 |
| `geometryIntent` | `semantic_extent` | 圈专家判定的对象语义范围，默认值 |
| `geometryIntent` | `reconstructed_extent` | 包含磨损、遮挡、抽象变形后的复原范围，必须单独评估 |
| `trainingRole` | `train` | 常规训练候选 |
| `trainingRole` | `validation` | gold 子集，训练脚本默认留出 |
| `trainingRole` | `holdout` | 保留在 IIML 中，正式 COCO 默认不输出 |

`annotationIssues[]` 用来记录为什么这条样本值得下一轮复核，受控值包括：

`low_contrast`、`texture_confusion`、`ambiguous_boundary`、`occluded_or_worn`、`oversegmented`、`undersegmented`、`class_uncertain`、`needs_expert_review`。

实务规则：

- 矩形、点、涂鸦默认 `weak`，不要求手描完整轮廓。
- SAM/YOLO 产物默认仍是 `candidate`；人工确认后可按质量改为 `silver` 或 `gold`。
- `gold + trainingRole=validation` 的样本应优先覆盖马、车、人物、神兽等核心类和常见难例。
- `geometryIntent=reconstructed_extent` 不能和 `visible_trace` 混在同一个评估指标里。
...
### 3.4 SAM / YOLO 候选必须人工审核后才能进训练池

- SAM 候选默认 `reviewStatus: candidate` ——必须改成 `reviewed` 或 `approved` 才会被 A2 导出按钮拉走
- YOLO bbox 可以作为 `annotationQuality=weak` 进入弱监督清单；若要进入 mask 训练，必须 **SAM 精修成 polygon → 人工审核 → silver/gold**
- 自动产物在训练池里都打 `provenance.author = "ai-then-human"` 标记

### 3.5 frame 与训练池规则（v0.3 新增）
...
| `reviewed` | 一次人工审核通过 | **进**（v0.3.1 起放宽） |
| `approved` | 二次审核通过（同标员或他人） | **进** |
| `rejected` | 任意阶段被拒 | **不进** |
| 无（历史手工标注） | 旧数据可能无该字段 | **进**（视为 reviewed） |
...
9. ✅ `reviewStatus` ∈ {`reviewed`, `approved`, undefined（历史手工）}
10. ✅ `inscription` 类时 `semantics.inscription.transcription` 非空
11. ✅ 多边形顶点 ∈ [6, 200]，BBox 面积 ≥ 64 px²
12. ⚠️ `annotationQuality=weak`、`geometryIntent=reconstructed_extent`、`trainingRole=validation/holdout`
    会进入报告和专用 manifest，但不作为硬错误
13. ✅ `category in {figure-loyal-assassin, figure-filial-son, figure-virtuous-woman}` 时
    **强烈建议**填 motif（因为这三类的学术价值就在于具体故事识别，没有 motif 是浪费样本）

不满足 1-11 的 annotation 仍保留在 IIML，只是不进 `data/datasets/wsc-han-stone-v0/`。
12-13 是 warning，导出仍会通过但报告会标记。`trainingRole=holdout` 会保留在 IIML / 报告中，正式 COCO 默认不输出。
...
def is_training_ready(ann: IimlAnnotation, doc: IimlDocument) -> tuple[bool, list[str]]:
    ...
    if ann.annotationQuality not in ("weak", "silver", "gold"):
        return False, ["bad-annotation-quality"]
    if ann.geometryIntent not in (None, "visible_trace", "semantic_extent", "reconstructed_extent"):
        return False, ["bad-geometry-intent"]
    if ann.trainingRole not in (None, "train", "validation", "holdout"):
        return False, ["bad-training-role"]
...
- [ ] 选了正确的 `annotationQuality`（weak / silver / gold）？
- [ ] `geometryIntent` 是否表达清楚：可见刻痕、语义范围，还是复原范围？
- [ ] gold 样本是否标了合适的 `trainingRole`（train / validation / holdout）？
- [ ] 如果 SAM 失败或边界不稳，是否补了 `annotationIssues[]`？
...
├── DATASET_CHANGELOG.md            每次冻结版本的变更记录与发布 checklist
...
│   ├── weak_annotations.json       bbox / point / scribble / weak tier 弱监督清单
│   ├── gold_validation.json        gold / validation 子集索引
│   ├── enhancement_manifest.json   RGB / 边缘 / normal / depth / relighting 多通道规划
│   ├── baseline_recipes.json       检测 / 实例分割 / 弱监督三条 baseline 配方
│   ├── active_learning_queue.json  下一轮人工复核优先队列
│   └── splits/
...
```jsonc
{
  ...
  "extension": {
    ...
    "iiml_annotationQuality": "gold",
    "iiml_geometryIntent": "semantic_extent",
    "iiml_trainingRole": "validation",
    "iiml_annotationIssues": ["ambiguous_boundary"],
...
**当前策略**：catalog 不再尝试把 Markdown metadata 与模型做模糊配对，石头列表直接由
`temp/` 模型目录生成，metadata 只作为统计和后续人工补录来源。旧版
`data/catalog.override.json` 保留为空配置以兼容已有仓库文件，但不再建议作为日常工作流。

**自检接口**：

- `GET /api/catalog/health` — 当前模型目录统计、孤儿模型列表、numericKey 冲突
- `GET /api/preflight` — 上面这份 + pic 配对 + IIML 完整度 + 训练池估算

**工作流**（批量标注前必查）：

1. `POST /api/scan/refresh` 强制重扫
2. `GET /api/catalog/health` → `numericKeyConflictCount: 0` 即可放心进入批量标注
3. 若仍有冲突，优先修正 `temp/` 模型文件命名或在 PIC 绑定阶段显式选择正确 stone。

### 14.5 双面石头的高清图关联（v0.8.x 新增）

部分石头（如武氏祠 16、17 号）两面均有画像内容。"关联"工作台为同一块
stone 提供两个固定 face slot：

| 面标识 | 文件名规则 | 用途 |
| --- | --- | --- |
| **A 面（主面）** | `{N}{name}{ext}`（无后缀） | 进入标注 / SAM / YOLO pipeline |
| **B 面（副面）** | `{N}-B{name}{ext}` | 仅留档；BindingWorkspace 可查看，不进 pipeline |

> 后端 `normalizeFace` 接受 B-Z 任意单字母，存储层支持多面；UI 当前固定
> 展示 A/B 两个 slot。如果将来出现三面以上的石头，把
> `BindingWorkspace.tsx::ALL_FACE_SLOTS` 扩成 `["", "B", "C"]` 即可。

**规则**：
- 一块 stone 只有一张主面进入标注（ai-service `_find_source_image`
  优先取主面；缺主面才 fallback 到字典序首个副面）
- 副面文件名带 `-{F}` 后缀，与主面共存于 `pic/`，`scanPicDir` 不视为
  numericKey 冲突
- 主面、副面可独立 bind / unbind，`data/pic-bindings.json` 用 `face` 字段
  区分（缺省 = 主面）

**操作**：在"关联" tab 选中 stone → 中栏显示 A、B 两个 slot。点击任一
slot 选中该面，再去右侧选未关联文件，点"关联到 A 面 / B 面"。

**已绑文件查看**：右栏切换"已关联"过滤模式，点击任一卡片即联动定位到
对应 stone + face slot，可在中栏查看大图或解除关联。
