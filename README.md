...
（Python + FastAPI + MobileSAM + SAM3 + YOLOv8）三套进程组成，全部本地运行，
数据不出本机。
...
- **SAM3 概念分割**：S3 工具可用中文预置词（人物 / 马 / 鸟 / 兽 / 车 / 纹饰）
  驱动 SAM3 文本概念分割，支持阈值、最大候选数和提示词自动扩展。
...
- **5 种学术导出**：IIML / CSV / COCO（喂 YOLO 训练）/ IIIF Web Annotation /
  `.hpsml` 自定义研究包
- **训练池导出闭环**：列表页可预检并导出 `data/datasets/wsc-han-stone-v0/`，
  输出 COCO train/val/test、IIML 备份、主动学习队列和逐条准入报告；导出后可
  一键打开训练集目录。
...
>
> SAM3 是 gated 模型。推荐把已授权下载的 `sam3.pt` 放到
> `ai-service/weights/sam3/sam3.pt`，或运行 `npm run setup:sam3` 检查 CUDA 并导入权重。
>
...
| `npm run setup:sam3` | 检查 / 导入 / 下载 SAM3 权重到 `ai-service/weights/sam3/sam3.pt` |
...
- **AI 服务**：Python + FastAPI + Pillow / numpy / OpenCV / MobileSAM / SAM3 /
  ultralytics；提供 SAM 多 prompt 智能分割、SAM3 文本概念分割、YOLOv8n 批量
  目标检测、5 种 Canny 线图、高清图 tif → PNG 转码缓存等。
...
| SAM3 权重 | `sam3.pt`（gated repo，建议手动放置或 `npm run setup:sam3`） | `./ai-service/weights/sam3/sam3.pt` | 入库 `.gitignore` |
...
| 训练池导出 | COCO train/val/test、IIML 备份、报告与主动学习队列 | `./data/datasets/wsc-han-stone-v0/` | 运行产物，入库 `.gitignore` |
...
GET    /api/preflight                       pic / IIML / 训练池上线前预检
...
POST   /ai/sam3                         SAM3 文本概念分割（人物 / 马 / 鸟 / 车 / 纹饰等）
...
- **多选合并**（polygon-clipping union）：候选 / 列表两个 tab 都支持勾选 ≥ 2 条
  ...
  继承
- **SAM3 候选**：工具栏 S3 弹出侧边面板，中文预置词映射英文概念词；人物默认
  走 `human figure / figure` 等扩展提示，适配画像石中非摄影风格的人物轮廓。

#### 训练池导出

- 列表 tab 底部显示本石头训练池 ready / warning / blocked 统计。
- `预检` 会汇总 pic 配对、IIML 字段完整度、训练池准入估算和样本不足类别。
- `导出训练集` 会重写 `data/datasets/wsc-han-stone-v0/`，生成：
  - `annotations/coco_train.json` / `coco_val.json` / `coco_test.json`
  - `annotations/active_learning_queue.json`
  - `reports/export_*.csv`
  - `stats.json`
  - `iiml/*.iiml.json`
- `reports/export_*.csv` 是排查训练池的首选文件：`bad-category`、`no-terms`、
  `pre-iconographic-too-short`、`iconographic-too-short`、`review-status-candidate`
  等原因会逐条列出。
- 证据源缺失现在是 warning，不再阻止视觉训练数据进入训练池。
- 导出成功后状态条右侧出现"目录"按钮，可直接打开本机训练集目录。
...
> [`docs/WORK_LOG_sam3_training_pool_2026-05-08.md`](docs/WORK_LOG_sam3_training_pool_2026-05-08.md)。
