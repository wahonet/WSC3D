...
> 当前版本：`v0.3.0` —— AI 标注闭环 · 多源底图 · 4 点对齐校准
...
- **标注**：基于 IIML 的图像志标注工作台，**3D 模型 / 高清原图双底图**自由切换，统一的 modelBox UV / 图像 UV 坐标系；矩形 / 椭圆 / 点 / 钢笔 / **SAM 智能分割**五种工具，候选可多选并集合并；4 点单应性**对齐校准**让两种底图下的标注双向投影。
...
- **AI 服务**：Python + FastAPI + Pillow/numpy/OpenCV/MobileSAM，提供 SAM 智能分割（高清图直读 + 当前视角截图双路径）、高清图 tif→PNG 转码缓存等。
...
POST /ai/sam                          SAM 智能分割（imageBase64 截图 / stoneId 高清图两条路径）
POST /ai/yolo                         YOLO 候选检测（占位）
POST /ai/canny                        OpenCV 线图（占位）
...
- **SAM 智能分割**：单击对象自动出多边形候选；优先走高清图路径（`stoneId` 直读 + MobileSAM ViT-T），无高清图时 fallback 到当前视角截图
...
- **M3 剩余**：YOLO 候选检测、AI 线图（Canny / Relic2Contour）、多解释并存与标注间关系（IIML relations）、共现术语推荐、知识图谱可视化（Cytoscape.js）。
- **M4**：多资源版本切换（原图 / RTI / 拓片 / 线图 / 法线图）、IIIF Web Annotation / COCO JSON / `.hpsml` 自定义研究包导出。
- **工程**：高清图缩略 / 多分辨率切片，主 chunk 进一步拆分到 < 600 KB。
