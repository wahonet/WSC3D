...
const bundle = await build({
  stdin: {
    contents: `
      ...
      export { useWorkspaceAuth } from './src/store/useWorkspaceAuth';
      export { solveSimilarity } from './src/lib/geometry';
...
const context = vm.createContext({
  ...
  history: {
    replaceState: (_, __, url) => { location.hash = url.startsWith('#') ? url : '' },
    pushState: (_, __, url) => { location.hash = url.startsWith('#') ? url : '' },
  },
  window: {
...
const { useApp, useWorkspaceAuth, solveSimilarity, useShortcuts, annotationReferences, hasReferences, referenceSource,
...
for (const destination of pages.filter(page => page !== 'library')) {
  location.hash = '#view=research&p=library&src=description&off=12&seg=7&fig=9'
  state().setPage(destination)
  for (const key of ['src', 'off', 'seg', 'fig']) assert.equal(hash().has(key), false)
  assert.equal(hash().get('view'), 'research', 'Submodule navigation preserves the unified workspace')
}
...
state().setPage('segment')
state().setTool('segment')
useWorkspaceAuth.setState({ authenticated: true })
location.hash = '#section=research&a=1&p=segment&lib=link&doc=88&pg=99&q=old&fig=19&src=layer%3A3&off=23'
openAnnotationReference(segmentRef)
assert.equal(hash().get('section'), 'library', 'Book references open the dedicated document centre')
assert.equal(state().tool, 'select', 'Opening a citation exits active image tools')
for (const [key, value] of Object.entries({ a: '1', lib: 'books', doc: '3', pg: '5', seg: '7' })) {
...
assert.equal(hash().get('section'), 'research', 'Descriptions require the research article, not the books-only centre')
assert.equal(hash().get('p'), 'library')
...
for (const key of ['seg', 'fig', 'q', 'doc', 'pg']) assert.equal(hash().has(key), false)
...
assert.equal(hash().get('section'), 'library', 'Adding a book reference opens the current library centre')
...
useWorkspaceAuth.setState({ authenticated: false })
location.hash = '#section=archive&stone=武011'
openAnnotationReference(segmentRef)
assert.equal(hash().get('section'), 'archive', 'Visitors stay on the public page')
assert.equal(dispatched.at(-1).type, 'wsc:public-reference')
assert.equal(dispatched.at(-1).detail.documentId, 3)

const points = [[0, 0], [100, 0], [0, 100], [100, 100]]
for (const [left, right] of [[points.map(() => [100, 100]), points], [points, points.map(() => [0, 0])],
  [[], []], [points.slice(1), points], [[[NaN, 0], ...points.slice(1)], points]]) {
  assert.equal(solveSimilarity(left, right), null, 'Degenerate/invalid pairs must disable saving')
}
const solved = solveSimilarity(points.map(([x, y]) => [-2 * y + 10, 2 * x - 20]), points)
assert.ok(solved)
assert.ok(Math.abs(solved.s - 2) < 1e-10)
assert.ok(Math.abs(solved.theta_deg - 90) < 1e-10)
assert.ok(solved.rmse < 1e-10)
