...
用法：
    tools/run.ps1 tests/ocr_worker_smoke.py ndl    <页图或PDF> [页号]
    tools/run.ps1 tests/ocr_worker_smoke.py mineru <PDF> [起页] [止页] [--backend hybrid-auto-engine|vlm-auto-engine|pipeline]
解释器与引擎目录按 app/config.py 的默认值（本机缓存中的 OCR 环境、src/engines/ndl），可用环境变量覆盖。
"""
import tempfile
import json
import os
...
smoke_root = Path(os.environ.get('WSC_SMOKE_OUTPUT', str(Path(tempfile.gettempdir()) / 'wsc-tests/ocr')))
out_dir = smoke_root / engine / (backend or 'default')
out_dir.mkdir(parents=True, exist_ok=True)
worker = ROOT / "src/backend" / "app" / "ocr_worker.py"
proc = subprocess.Popen([py, "-X", "utf8", str(worker), "--engine", engine, "--ndl-root", str(settings.ndl_root),
                         "--models-dir", str(settings.ndl_models_dir if engine == 'ndl' else settings.mineru_models_dir)],
...
report_path = smoke_root / ('ocr-' + engine + '-' + (backend or 'default') + '.json')
report_path.parent.mkdir(parents=True, exist_ok=True)
report_path.write_text(json.dumps({'engine':engine,'backend':backend or 'default','python':py,'input':target,'boot':boot,'result':r},ensure_ascii=False,indent=2),encoding='utf-8')
call({"cmd": "shutdown"})
