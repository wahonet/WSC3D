...
class AnnotationReferenceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.stone = Stone(id="武901", code="武901", name="测试石", dirname="test", description="甲段文字。乙段文字。丙段文字。")
        ...
        self.app.include_router(library_router, prefix="/api")
        self.app.include_router(alignment_router, prefix="/api")
    ...
    def test_save_excerpt_reviews_candidate_and_links_back_from_original_page(self):
        source = self.pages[0].segments[0]
        source.text = "𠀀一人持弓，向右射箭。身后站立一人。"
        self.node.review_status = "candidate"
        self.db.commit()
        linked = self.add({"kind": "segment", "segment_id": source.id})
        self.assertEqual(linked["review_status"], "candidate")
        ref = linked["references"][0]
        excerpt = {"reference_id": ref["id"], "field": "pre_iconographic", "start": 1, "end": 5, "text": "一人持弓"}
        saved = self.client.patch(f"/api/annotations/{self.node.id}", json={
            "label": "持弓人物", "review_status": "reviewed", "semantics": {"pre_iconographic": "一人持弓"},
            "reference_excerpts": [excerpt],
        })
        self.assertEqual(saved.status_code, 200, saved.text)
        value = saved.json()
        self.assertEqual(value["review_status"], "reviewed")
        self.assertEqual(value["geometry"], linked["geometry"])
        self.assertEqual(value["note"], linked["note"])
        self.assertEqual(value["references"][0]["text"], source.text)
        self.assertEqual(value["references"][0]["excerpts"][0]["text"], "一人持弓")
        page = self.client.get(f"/api/library/pages/{source.page_id}/annotations")
        self.assertEqual(page.status_code, 200, page.text)
        self.assertEqual(page.json()[0]["annotation"]["id"], self.node.id)
        self.assertEqual(page.json()[0]["references"][0]["excerpts"][0]["start"], 1)
        # 重复保存同一句不堆积引用，也不新建图像标注。
        again = self.client.patch(f"/api/annotations/{self.node.id}", json={
            "semantics": {"pre_iconographic": "一人持弓"}, "reference_excerpts": [excerpt],
        })
        self.assertEqual(len(again.json()["references"]), 1)
        self.assertEqual(len(again.json()["references"][0]["excerpts"]), 1)

    def test_invalid_or_foreign_excerpt_cannot_partially_save_annotation(self):
        source = self.pages[0].segments[0]
        other = self.add({"kind": "segment", "segment_id": source.id}, self.other)
        own = self.add({"kind": "segment", "segment_id": source.id})
        for ref_id, snippet in [(other["references"][0]["id"], source.text[:4]), (own["references"][0]["id"], "伪造文字")]:
            response = self.client.patch(f"/api/annotations/{self.node.id}", json={
                "label": "不应保存", "semantics": {"iconographic": snippet},
                "reference_excerpts": [{"reference_id": ref_id, "field": "iconographic", "start": 0, "end": 4, "text": snippet}],
            })
            self.assertEqual(response.status_code, 422)
            self.assertEqual(self.node.label, "节点甲")
            self.assertEqual(self.node.references[0].excerpts, [])

    def test_page_backlink_does_not_retarget_after_source_identity_changes(self):
        source = self.pages[0].segments[0]
        self.add({"kind": "segment", "segment_id": source.id})
        self.assertEqual(len(references.page_annotations(self.db, self.pages[0])), 1)
        source.reference_identity = "a-different-ocr-source"
        self.db.commit()
        self.assertEqual(references.page_annotations(self.db, self.pages[0]), [])
        self.assertTrue(annotation_out(self.node).references[0].source_missing)

    def test_merging_regions_keeps_selected_excerpts_from_same_source(self):
        source = self.pages[0].segments[0]
        self.add({"kind": "segment", "segment_id": source.id})
        self.add({"kind": "segment", "segment_id": source.id}, self.other)
        self.node.references[0].excerpts = [{"field": "pre_iconographic", "start": 0, "end": 3, "text": source.text[:3]}]
        self.other.references[0].excerpts = [{"field": "iconographic", "start": 3, "end": 6, "text": source.text[3:6]}]
        self.db.commit()
        structure.adopt_geometry(self.db, self.node, self.other)
        self.db.commit()
        self.assertEqual(len(self.node.references), 1)
        self.assertEqual(len(self.node.references[0].excerpts), 2)


    def test_batch_delete_nested_parents_keeps_nearest_surviving_ancestor(self):
        # Parent IDs need not precede their children after a user reparents nodes.
        for offset, ids in [(200, [201, 203]), (300, [303, 301])]:
            with self.subTest(ids=ids):
                for aid, parent in [(offset, None), (offset + 3, offset),
                                    (offset + 1, offset + 3), (offset + 2, offset + 1)]:
                    self.db.add(Annotation(id=aid, stone_id=self.stone.id, asset_id=self.asset.id,
                                           tool='annotate', atype='none', geometry={}, parent_id=parent, label=str(aid)))
                self.db.commit()
                response = self.client.post('/api/annotations/batch-delete', json={'ids': ids})
                self.assertEqual(response.status_code, 200, response.text)
                self.db.expire_all()
                self.assertEqual(self.db.get(Annotation, offset + 2).parent_id, offset)
                self.assertTrue(all(self.db.get(Annotation, aid) is None for aid in ids))

    def test_batch_delete_whole_ancestry_promotes_survivor_to_root(self):
        leaf = self.make_node('叶节点')
        self.other.parent_id, leaf.parent_id = self.node.id, self.other.id
        self.db.commit()
        response = self.client.post('/api/annotations/batch-delete', json={'ids': [self.other.id, self.node.id]})
        self.assertEqual(response.status_code, 200, response.text)
        self.db.refresh(leaf)
        self.assertIsNone(leaf.parent_id)

    def test_adopting_direct_or_indirect_ancestor_rejects_without_changes(self):
        leaf = self.make_node('叶节点')
        self.other.parent_id, leaf.parent_id = self.node.id, self.other.id
        self.desc(node=self.node)
        self.db.commit()
        for target in [self.other, leaf]:
            before = dict(target.geometry)
            response = self.client.post(f'/api/annotations/{target.id}/adopt', json={'source_id': self.node.id})
            self.assertEqual(response.status_code, 422, response.text)
            self.db.expire_all()
            self.assertEqual(target.geometry, before)
            self.assertIsNotNone(self.db.get(Annotation, self.node.id))
            self.assertEqual(len(self.node.references), 1)
            self.assertNotEqual(target.parent_id, target.id)

    def test_adopting_a_descendant_preserves_valid_children(self):
        leaf = self.make_node('叶节点')
        self.other.parent_id, leaf.parent_id = self.node.id, self.other.id
        self.db.commit()
        response = self.client.post(f'/api/annotations/{self.node.id}/adopt', json={'source_id': self.other.id})
        self.assertEqual(response.status_code, 200, response.text)
        self.db.refresh(leaf)
        self.assertEqual(leaf.parent_id, self.node.id)
        self.assertIsNone(self.node.parent_id)

    def alignment_assets(self):
        self.asset.width = self.asset.height = 200
        self.asset.extra = {'is_master': True, 'align_to_master': {
            's': 1, 'theta_deg': 0, 'tx': 0, 'ty': 0, 'master_asset_id': self.asset.id}}
        photo = Asset(stone_id=self.stone.id, kind='photo', filename='other.jpg', relpath='other.jpg',
                      width=200, height=200, extra={'align_to_master': {
                          's': 2, 'theta_deg': 0, 'tx': 10, 'ty': 20, 'master_asset_id': self.asset.id}})
        self.db.add(photo)
        self.db.commit()
        return photo

    def test_realign_to_master_on_right_preserves_identity_and_projection(self):
        from app.services import alignment
        photo = self.alignment_assets()
        with patch('app.services.resource_versions.pin_asset'):
            alignment.commit_alignment(self.db, self.stone.id, photo, self.asset,
                                       {'s': .4, 'theta_deg': 0, 'tx': -5, 'ty': -10})
        self.db.refresh(self.asset)
        self.assertEqual(self.asset.chain, {'s': 1, 'theta_deg': 0, 'tx': 0, 'ty': 0, 'master_asset_id': self.asset.id})
        self.assertTrue(self.asset.is_master)
        self.assertEqual([photo.chain[k] for k in ['s', 'tx', 'ty']], [2.5, 12.5, 25])
        projection = alignment.project_annotations(self.db, photo)
        self.assertTrue(projection.ok)
        node = next(item for item in projection.items if item.id == self.node.id)
        self.assertAlmostEqual(node.geometry['points'][0][0], .015)
        self.assertAlmostEqual(node.geometry['points'][0][1], -.01)

    def test_master_on_left_and_unaligned_pair_behave_as_before(self):
        from app.services import alignment
        photo = self.alignment_assets()
        with patch('app.services.resource_versions.pin_asset'):
            alignment.commit_alignment(self.db, self.stone.id, self.asset, photo,
                                       {'s': .5, 'theta_deg': 0, 'tx': 3, 'ty': 4})
            self.assertEqual([photo.chain[k] for k in ['s', 'tx', 'ty']], [.5, 3, 4])
            self.assertEqual(self.asset.chain['s'], 1)
            photo.extra = {}; self.asset.extra = {}; self.db.commit()
            result = alignment.commit_alignment(self.db, self.stone.id, self.asset, photo,
                                                {'s': 1, 'theta_deg': 0, 'tx': 0, 'ty': 0})
        self.assertTrue(result.warning)
        self.assertIsNone(photo.chain)
        self.assertIsNone(self.asset.chain)

    def test_invalid_alignment_is_rejected_before_any_snapshot_or_database_write(self):
        photo = self.alignment_assets()
        count = self.db.query(Annotation).count()
        for field, value in [('s', 0), ('s', -1), ('s', True), ('s', 'nan'), ('s', 'inf'),
                             ('s', 1e-320), ('theta_deg', 'inf'), ('tx', 'nan'), ('ty', None), ('rmse_px', -1)]:
            with self.subTest(field=field, value=value), patch('app.services.resource_versions.pin_asset') as pin:
                geom = {'s': 1, 'theta_deg': 0, 'tx': 0, 'ty': 0, field: value}
                response = self.client.post('/api/align/commit', json={'stone_id': self.stone.id,
                    'left_asset_id': self.asset.id, 'right_asset_id': photo.id, 'geometry': geom})
                self.assertEqual(response.status_code, 422, response.text)
                pin.assert_not_called()
                self.assertEqual(self.db.query(Annotation).count(), count)
                self.assertEqual(photo.chain['s'], 2)
                self.assertEqual(self.asset.chain['s'], 1)
