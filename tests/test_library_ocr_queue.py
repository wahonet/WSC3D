...
from run_library_ocr import Api, QueueBlocked, QueueRunner, all_pages, state_lock
...
class ApiAuthenticationTests(unittest.TestCase):
    ...
    def test_public_reads_and_first_write_login(self):
        api = Api('http://127.0.0.1:8030')
        api.session = Mock()
        api.session.open.side_effect = [self.response([]), self.response({'authenticated': True}),
                                        self.response({'running': True})]
        self.assertEqual(api.get('/documents'), [])
        with patch.dict('os.environ', WSC_WORKSPACE_PASSWORD='test-password'):
            self.assertEqual(api.post('/documents/1/ocr', {'pages': [1]}), {'running': True})
        calls = api.session.open.call_args_list
        self.assertEqual([c.args[0].full_url for c in calls], [
            api.base_url + '/api/library/documents', api.base_url + '/api/workspace/login',
            api.base_url + '/api/library/documents/1/ocr'])
        self.assertEqual(json.loads(calls[1].args[0].data)['password'], 'test-password')
        self.assertTrue(api.signed_in)

    def test_expired_session_is_renewed_and_retried_once(self):
        api = Api('http://127.0.0.1:8030')
        api.signed_in = True
        api.session = Mock()
        api.session.open.side_effect = [HTTPError(api.base_url, 401, 'Expired', {}, None),
            self.response({'authenticated': True}), self.response({'running': True})]
        self.assertEqual(api.post('/documents/1/ocr', {'pages': [1]}), {'running': True})
        calls = api.session.open.call_args_list
        self.assertIs(calls[0].args[0], calls[2].args[0])
        self.assertEqual(calls[1].args[0].full_url, api.base_url + '/api/workspace/login')

    def test_non_authentication_failures_do_not_resubmit_jobs(self):
        for failure in [TimeoutError('unknown submission result'),
                        HTTPError('http://127.0.0.1', 500, 'Internal error', {}, None)]:
            with self.subTest(failure=failure):
                api = Api('http://127.0.0.1:8030')
                api.signed_in = True
                api.session = Mock()
                api.session.open.side_effect = failure
                with self.assertRaises(type(failure)):
                    api.post('/documents/1/ocr', {'pages': [1]})
                self.assertEqual(api.session.open.call_count, 1)

    def test_repeated_unauthorized_response_is_not_an_infinite_retry(self):
        api = Api('http://127.0.0.1:8030')
        api.signed_in = True
        api.session = Mock()
        api.session.open.side_effect = [HTTPError(api.base_url, 401, 'Expired', {}, None),
            self.response({'authenticated': True}), HTTPError(api.base_url, 401, 'Denied', {}, None)]
        with self.assertRaises(HTTPError):
            api.post('/documents/1/ocr', {'pages': [1]})
        self.assertEqual(api.session.open.call_count, 3)
